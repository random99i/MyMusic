...
function getPlayList(){
    var list = $("#music_list");
    $.get("/MusicPro/singer/opensong?so_id="+so_id,function(data){
    		if(data!=null){
    			alert(data[0].link_lrc);
	            list.empty();
...
function initPlayCtrl(){
    ...
    playCtrl.find(".vol").bind("click",function(){
    	//alert("hhh");
    	var mvol=$(".m-vol");
    	//alert(mvol.css("visibility"));
    	if(mvol.css("visibility")=="hidden"){
    		mvol.css("visibility","visible");
    	}else{
    		mvol.css("visibility","hidden");
    	}
    });
}

//音量调节
var curr=$(".curr"); //80 284 364
var btnn=$(".btn");
curr.progress = 0;
curr.slideable = true;
curr.setProgress = function(val){
	btnn.css({width:val+"%"});
	curr.progress=val;
}
curr.bind("click",function(e){
	//alert("h");
	
	var y=e.screenY;
	//alert(y);
	//alert(!curr.slideable)
		//if(!curr.slideable)return;
	var progress=Math.round((400-y)*10/8);
	//alert(progress);
	curr.setProgress(progress);
	curr.trigger("changer");
})

//播放模式
$(".loop").click(function(){//单曲循环
    $(".single").show();
    $(".loop").hide();
    $(".random").hide();    
...
$(".single").click(function(){//随机播放
    ...
    $player.attr("loop","");
    alert($player.attr("loop"));
    var list_len=$player.playList.all().length;
    //alert(list_len);
    var rand_num=Math.floor(Math.random()*list_len);
    $player.play(rand_num);
});
